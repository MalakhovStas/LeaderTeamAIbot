...
class MessageGetOpenQuestionsIrritation(BaseMessage):
    ...
    async def _set_answer_logic(self, update: Message, state: Optional[FSMContext] = None):
        ...
        try:
            ...
            user.seven_petals.optimism = state_data.get("optimism", 0)
            user.seven_petals.stream = state_data.get("stream", 0)
            user.seven_petals.sense = state_data.get("sense", 0)
            user.seven_petals.love = state_data.get("love", 0)
            user.seven_petals.play = state_data.get("play", 0)
            user.seven_petals.study = state_data.get("study", 0)
            user.seven_petals.impact = state_data.get("impact", 0)
