"""Модуль обработчиков входящих сообщений администраторов"""
...
@dp.message_handler(commands=['my_id', 'mailing', 'commands', 'how_users',
                              ...
                              'unload_payment_data_user', 'load_stat', 'check_proxies'], state='*')
...
async def admins_in_password_handler(message: Message, state: FSMContext) -> None:
    """Обработчик входящих сообщений в состоянии пользователя FSMAdminStates.password_mailing"""
...
@dp.message_handler(state=FSMAdminStates.mailing_admins, content_types=[
    'text', 'photo', 'audio', 'voice', 'video', 'video_note', 'document', 'sticker'])
async def admins_mailing_handler(message: Message, state: FSMContext) -> None:
    """Обработчик входящих сообщений в состоянии пользователя FSMAdminStates.password_mailing
    с перечисленными типами данных"""
    text, next_state = await adm.mailing(update=message, only_admins=True)
    await state.set_state(next_state) if next_state else await state.reset_state()
    # await bot.send_message(chat_id=message.from_user.id, text=text)


@dp.message_handler(state=FSMAdminStates.mailing, content_types=[
    'text', 'photo', 'audio', 'voice', 'video', 'video_note', 'document', 'sticker'])
async def users_mailing_handler(message: Message, state: FSMContext) -> None:
    """Обработчик входящих сообщений в состоянии пользователя FSMAdminStates.mailing
    с перечисленными типами данных"""
...
async def func_change_user_requests_balance(message: Message, state: FSMContext):
    ...
    if type_result == 'document':
        ...
        await bot.send_message(
            chat_id=message.from_user.id, text=result, disable_web_page_preview=True)
