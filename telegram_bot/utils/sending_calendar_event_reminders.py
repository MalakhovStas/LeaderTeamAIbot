...
from telegram_bot.config import FACE_BOT
            ...
            async for member in reminder.event.company.members.all():
                async for tg_account in member.tg_accounts.all():
                    await bot.send_message(
                        ...
                        text=f'{FACE_BOT} Через {dict(CalendarEventReminder.INTERVAL_CHOICES)[reminder.interval]} - {reminder.event.event_date.astimezone().strftime(settings.GENERAL_DATETIME_FORMAT_FOR_MESSAGE)}\nсостоится событие: <b>"{reminder.event.title}"</b>\n{reminder.event.description}'
                    )
