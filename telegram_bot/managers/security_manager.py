...
class SecurityManager:
    ...
    async def flood_control(self, update: Union[CallbackQuery, Message]) -> str:
        ...
        if not flood_data:
            ...
            elif flood_data.num_alerts == settings.FLOOD_CONTROL_NUM_ALERTS:
                flood_data = flood_data._replace(
                    num_alerts=0, time_block=time.time() + settings.FLOOD_CONTROL_STOP_TIME)
                result = 'block'

            elif time.time() - flood_data.last_time < settings.FLOOD_CONTROL_TIME:
                flood_data = flood_data._replace(
